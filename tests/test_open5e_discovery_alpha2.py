...
def test_open5e_discovery_does_not_register_or_authorize_source(monkeypatch, tmp_path: Path):
    discovery_dir=tmp_path/'data'/'discovery'/'open5e'
    ...
    fake_source_settings=SimpleNamespace(sources_dir=sources_dir)
    monkeypatch.setattr(open5e,'settings',fake_open5e_settings)
    monkeypatch.setattr(source_module,'settings',fake_source_settings)
    assert source_module.discover_source_manifests()==[]
    client=Open5eClient(base_url=fake_open5e_settings.open5e_base_url,transport=httpx.MockTransport(_handler))
    result=discover_open5e(client)
    assert result['imported_into_archie'] is False
    assert source_module.discover_source_manifests()==[]
    assert (discovery_dir/'latest.json').exists()
