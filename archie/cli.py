...
def main(argv=None):
    ...
    s=sub.add_parser('search'); s.add_argument('query'); s.add_argument('--top-k',type=int,default=8); s.add_argument('--json',action='store_true')
    ...
    try:
        if args.cmd=='verify-source': print(json.dumps(verify_source(),indent=2))
            ...
            if args.json: print(json.dumps([r.to_dict() for r in rows],indent=2))
                ...
                for r in rows:
                    print(f"[{r.evidence_id}] PDF p.{r.page_pdf} | {r.heading}\n{r.text[:900]}\n---")
        elif args.cmd=='ask':
            raw=None
